...
OUT_DIR = PROJECT_ROOT / "data" / "raw" / "ercot"
...
def parse_ymd(date_str: str) -> date:
    """Parse YYYY-MM-DD into a date object with a clear error message."""
    try:
...
def validate_ymd_range(date_from: str, date_to: str) -> None:
    """Validate a closed date range in YYYY-MM-DD format."""
    start = parse_ymd(date_from)
    end = parse_ymd(date_to)
    if start > end:
        raise ValueError(f"Invalid date range: from {date_from} is after to {date_to}.")


def validate_positive_int(name: str, value: int, *, max_value: int | None = None) -> None:
    """Validate integer CLI parameters."""
    if value <= 0:
